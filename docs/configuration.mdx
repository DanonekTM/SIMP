...
| max_file_size      | string  | `1MB`                   | Maximum upload size per file (e.g., `1MB`).                                          |
...
| max_storage        | string   | `10MB`            | Maximum total storage allowed (e.g., `10MB`). |
...
| routes       | object  | `{ "/api/login": ... }`         | Per-route overrides (e.g., `/api/login`, `/api/upload`). |
